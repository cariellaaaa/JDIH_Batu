...
class StaffController extends Controller
{
    ...
    public function deleteprodukhukum($id)
    {
        try {
            ...
            $draft = StaffDokumentasi::findOrFail($id);

            // Hapus file terkait (jika ada file yang tersimpan di storage)
            if ($draft->ttd_walikota_salinan) {
                Storage::delete($draft->ttd_walikota_salinan);
            }
    ...
    public function editprodukhukum(Request $request, $id)
    {
        // Mengambil data produk hukum (StaffUndang) berdasarkan ID
        $staffUndang = StaffUndang::findOrFail($id);

        // Mendapatkan ID dari user yang sedang login
        $userId = Auth::user()->id;

        // Validasi: pastikan hanya staff yang ditugaskan (user_id) yang dapat mengedit
        if ($staffUndang->user_id != $userId) {
            // Jika user yang login tidak cocok, kembalikan dengan pesan error
            return redirect()->back()->with('error', 'Anda tidak memiliki izin untuk mengedit produk hukum ini.');
        }

        // Jika validasi lolos, tampilkan halaman edit produk hukum
        return view('auth.staff_perundang_undangan.editprodukhukum', [
            'draft' => $staffUndang
        ]);
    }
    ...
    public function katalogmonografihukum(Request $request)
    {
        return view('auth.staff_dokumentasi.katalogmonografihukum', [
            'staff_dokumentasi' => StaffDokumentasi::all(),
        ]);
    }
    ...
    public function addmonografihukum()
    {
        return view('auth.staff_dokumentasi.addmonografihukum', [
            'draftAll' => ProdukHukum::all(),
            'jenis' => Jenis::all()
        ]);
    }
